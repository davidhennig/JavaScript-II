...
runnersLargeShirt = runners.filter(function(size) {
...
let whoDonated = [];
runners.forEach(function(item) {
  return whoDonated.push(`${item.first_name} donated $${item.donation}`);
});
console.log(whoDonated);
// Problem 2
let whoWorksWhere = [];
runners.forEach(function(item) {
  return whoWorksWhere.push(
    `${item.first_name} ${item.last_name} works at ${item.company_name}`
  );
});
console.log(whoWorksWhere);
// Problem 3
let firstTen = [];
firstTen = runners.filter(function(item) {
  return item.id <= 10;
});
